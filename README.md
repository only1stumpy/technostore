...
- **Origin validation** для всех mutation endpoints через `validateOrigin()`.
- **Security headers**: `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, CSP для API routes.
...
**Важно:** `bun run build` выполняет только сборку приложения и **не применяет миграции** к базе данных.
...
Это разделение позволяет контролировать момент применения миграций и избежать изменения production БД во время сборки.
