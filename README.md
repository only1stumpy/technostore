...
**Demo-администратор:** `+79990000000`, код подтверждения доступен в mock-режиме SMS.
...
- **Rate limiting** отправки и проверки SMS-кодов.
...
### 4. Сгенерировать Prisma Client и ER-диаграмму
...
| `bun run build` | Генерация Prisma Client, применение миграций и production build |
| `bun run start` | Запуск production-сервера |
| `bun run lint` | Проверка ESLint |
| `bun run postinstall` | Генерация Prisma Client и ER-диаграммы после установки зависимостей |
...
1. генерируется Prisma Client и ER-диаграмма;
...
## Что не вошло в scope, но может быть добавлено

Текущий проект сфокусирован на базовом e-commerce цикле: каталог, авторизация, корзина, заказ и администрирование. В рамках дипломной версии сознательно не реализованы:

- онлайн-оплата картой и интеграция платежных шлюзов;
...
Архитектура проекта оставляет возможность добавить эти функции в будущем без переписывания основных модулей.
